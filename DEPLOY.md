...
**部署平台：Render.com**（对小白最友好，界面干净，自动给网址，没有烦人的限制）
...
- 一台能上网的电脑
- 一个常用邮箱
- 网站代码已经放在 GitHub 仓库 `xiongweibo0126/design`（公开仓库）
- 网站名字（先不用买域名，Render 会先给你一个免费网址，比如 `tokenbase.onrender.com`）
...
> 你现在的仓库叫 `design`（已经在 GitHub 上了）。Render 部署时直接用这个仓库即可。

---

## 第 2 步：在 Render 一键部署（免费，自动给你网址）

Render 是一台"云电脑"，会一直帮你运行这个网站。它识别 Node 项目，点几下就上线。

1. 打开 [render.com](https://render.com) ，点 **Sign Up** → 选 **Continue with GitHub**（用你同一个 GitHub 账号登录）。
2. 登录后，点右上角 **New → Web Service**。
3. 在 **Connect a repository** 里选 **GitHub**，找到 `xiongweibo0126/design` → 点 **Connect**。
   > 如果列表里找不到，说明仓库还是私有的。去 github.com 把 `design` 仓库改成 **Public**（Settings → Danger Zone → Change visibility → Make public）。
4. 在配置页照抄下面这栏：

   | 项目 | 填什么 |
   |---|---|
   | **Name** | `tokenbase` |
   | **Region** | 选 **Oregon (US)**（默认即可） |
   | **Branch** | `main` |
   | **Runtime** | **Node** |
...
5. 滑到最下面点 **Create Web Service**。
6. 等 1–3 分钟，Render 会自动构建并启动。完成后页面顶部显示一个网址，形如 `https://tokenbase.onrender.com` → 点它，**你的网站已经上线了！** 🎉
...
> **免费版注意**：Render 免费服务 15 分钟没人访问会自动"睡觉"，你第一次打开可能要等约 30 秒才加载出来，这是正常的，之后就快了。另外免费档可能要求绑一张信用卡做身份验证（不会扣费）。

---

## 第 3 步：让网站"真正能用"（填几把钥匙，没有就不填）

在 Render 项目里：点 **Environment**（环境变量）标签 → **Add Environment Variable**，按下面逐个添加。**不填的项直接跳过**，网站不会坏。
...
填完后，Render 会自动重新部署。再调模型就是**真实结果**了。
...
2. 在 Render 项目 → **Settings → Custom Domains**，填入你的域名，按提示去域名商后台加一条 **CNAME** 记录指向 Render 给你的地址。
3. 等几分钟，用你的域名访问即可。
...
1. **数据存哪里**：现在客户的余额存在服务器的一个文件里，网站重启可能清空。**正式做生意前**，要把存储换成数据库（Supabase / Neon，Render 里点一下就能加，我可以帮你改）。
...
## 卡住了怎么办
每一步截图发给我，我手把手带你点；或者你开着自己的 GitHub / Render 页面，我一步步说你点哪里。不用怕，全程不用写代码。
